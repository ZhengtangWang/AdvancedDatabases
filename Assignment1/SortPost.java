...
public class SortPost {
	...
	public void doSort() throws Exception {
		...
		String line; //creat string
		while ((line = in.readLine()) != null) { //read chunks
			chunk[cnt] = line;
			cnt++; 
			if (cnt == M) {
				sortAndSaveChunk(chunk, tmpfileprefix + numChunks); //save temp file
				numChunks++;
				chunk = new String[M]; //Empty the array
				cnt = 0;
			}
		}
		...
		for (;;) {
			...
			if (minh.head == null) { //poll is null
				break; //sublists are exhausted
			} else { 
				out.write(minh.head); //add head to output
				out.newLine();
				heads.add(new HeadIndexPair(readers[minh.i].readLine(), minh.i));
				//insert new head from sublist
			}
		}
